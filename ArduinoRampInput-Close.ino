...
//You Expected Input
int expectDiff = analogRead(A2) / 3.69; //range from -4 to 309
int prev_input = 0;
int len = 1;
int16_t kp = 2;
int16_t kd = 0.5;
...
void execute() {
  ...
  if (abs(prev_input - analogRead(A2)) > 20) {
    if (analogRead(A2) / 3.69 < 20) {
      setInput(0);
      MCP4725.setVoltage(MCP4725_value, false);
      i = 0;
    } else {
      setInput(analogRead(A2) / 3.69); //analogRead(A2)/3.69
      MCP4725.setVoltage(MCP4725_value, false);
      i = 0;
    }
    prev_input = analogRead(A2);
  }
